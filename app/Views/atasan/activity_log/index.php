    ...
    <div class="relative overflow-hidden bg-white rounded-3xl border border-gray-100 shadow-sm p-8">
        ...
        <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Log <span class="text-[#1D2F83]">Aktivitas Atasan</span></h1>
                    <div class="flex items-center gap-2 mt-1">
                        <span class="relative flex h-2 w-2">
                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-2 w-2 bg-blue-500"></span>
                        </span>
                        <p class="text-sm text-gray-500 font-medium">Monitoring Audit Trail Manajemen KINETRACK</p>
                    </div>
                </div>
            </div>

            <div class="hidden md:block">
                <div class="px-4 py-2 rounded-xl bg-blue-50 border border-blue-100 text-blue-700 text-xs font-bold uppercase tracking-wider shadow-sm">
                    Level Akses: Atasan
                </div>
            </div>
        </div>
        ...
        <div class="divide-y divide-gray-50">
            <?php if (empty($logs)): ?>
                <div class="py-20 text-center">
                    ...
                    <p class="text-sm text-gray-500">Sistem belum mencatat tindakan manajemen apa pun.</p>
                </div>
            <?php else: ?>
                    ...
                    <div class="relative flex flex-col md:flex-row md:items-start gap-6">
                        ...
                        <div class="flex-grow space-y-3">
                            <div class="flex flex-wrap items-center gap-3">
                                ...
                                <span class="text-[10px] px-2.5 py-0.5 rounded-lg border font-extrabold bg-blue-50 text-blue-700 border-blue-100 uppercase">
                                    Action: <?= esc($log['action'] ?? 'LOG') ?>
                                </span>
                            ...
                            <div class="relative">
                                ...
                                <p class="text-[13px] text-gray-600 leading-relaxed pl-4 italic font-medium">
                                    "<?= esc($log['description']) ?>"
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="md:text-right flex-shrink-0 flex md:flex-col items-center md:items-end gap-2 md:gap-0">
                            ...
                            <p class="text-[11px] text-gray-400 font-medium uppercase tracking-tighter"><?= date('H:i:s', strtotime($log['created_at'])) ?> WIB</p>
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
