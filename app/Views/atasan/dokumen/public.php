...
<div class="px-4 py-8 max-w-7xl mx-auto font-sans">
    ...
    <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-10">
        <div class="flex items-center gap-5">
            ...
            <div>
                <h1 class="text-3xl font-black text-blue-900 tracking-tight leading-none">Dokumen Publik</h1>
                <p class="text-[11px] text-slate-400 mt-2 font-black uppercase tracking-[0.2em]">Arsip Resmi Lintas Unit Kerja • Global Access</p>
            </div>
        </div>
    </div>
    ...
    <?php if (empty($dokumen)): ?>
        <div class="py-20 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2.5rem] text-center">
            <div class="flex flex-col items-center text-slate-300">
                ...
                <h4 class="text-lg font-bold uppercase tracking-widest">Belum Ada Dokumen Publik</h4>
                <p class="text-sm italic mt-1">Dokumen resmi yang dipublikasikan secara global akan muncul di sini</p>
            </div>
        </div>
    <?php else: ?>
            ...
            <?php foreach ($dokumen as $d): ?>
                <div class="archive-card p-7 group">
                    <div class="flex justify-between items-start mb-6">
                        ...
                        <span class="px-3 py-1.5 rounded-xl border bg-emerald-50 text-emerald-600 border-emerald-100 text-[9px] font-black uppercase tracking-widest shadow-sm">
                            Publik
                        </span>
                    </div>
                    ...
                    <div class="flex-grow">
                        ...
                        <p class="text-xs text-slate-500 line-clamp-2 leading-relaxed italic mb-4 min-h-[2.5rem]">
                            <?= esc($d['deskripsi'] ?: 'Tidak ada deskripsi tambahan untuk dokumen publik ini.') ?>
                        </p>
                        ...
                        <div class="space-y-2.5 mb-4 pt-4 border-t border-slate-50">
                            <div class="flex items-center justify-between text-[10px] font-bold text-slate-600">
                                ...
                                <span class="truncate ml-4 font-black"><?= esc($d['nama_pengirim'] ?? '-') ?></span>
                            </div>
                            <div class="flex items-center justify-between text-[10px] font-bold text-slate-600">
                            ...
                            <div class="flex items-center justify-between text-[10px] font-bold text-slate-600">
                                <span class="text-slate-400 uppercase tracking-tighter">Kategori</span>
                                <span class="truncate ml-4"><?= esc($d['nama_kategori'] ?? '-') ?></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 pt-6 border-t border-slate-50 flex items-center justify-between">
                        <div class="flex flex-col text-left">
                            <span class="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-1 leading-none">Tanggal Terbit</span>
    ...
    <div class="mt-16 pt-8 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 opacity-50 text-center md:text-left">
        <p class="text-[10px] text-slate-500 font-black tracking-widest uppercase">
            &copy; <?= date('Y') ?> KINETRACK — Politeknik Negeri Bandung • Global Public Repository
        </p>
    </div>
</div>
