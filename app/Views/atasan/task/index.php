                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="tw-item p-4 flex flex-col justify-between <?= $isOpen ? 'active' : '' ?>">
                            ...
                            <div>
                                <?php if (!$hasFilled): ?>
                                    <?php if ($isOpen): ?>
                                        <a href="<?= base_url('atasan/task/input/'.$ind['indikator_id'].'/'.$tw) ?>" class="btn-soft btn-blue">Isi Data</a>
                                    <?php else: ?>
                                        <span class="btn-soft bg-slate-50 text-slate-300 cursor-not-allowed">Terkunci</span>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <?php if ($percent >= 100): ?>
                                        <a href="<?= base_url('atasan/task/report/'.$ind['indikator_id'].'/'.$tw) ?>" target="_blank" class="btn-soft btn-outline">Laporan</a>
                                    <?php else: ?>
                                        <a href="<?= base_url('atasan/task/progress/'.$ind['indikator_id'].'/'.$tw) ?>" class="btn-soft bg-amber-50 text-amber-600">Progres</a>
                                    <?php endif; ?>
                                <?php endif; ?>
                            </div>
                        </div>
