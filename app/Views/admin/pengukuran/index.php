...
<script>
// ============================
// UPDATE TRIWLUN BERDASARKAN TAHUN
// ============================
document.querySelector('#tahun_id').addEventListener('change', async function(){
    ...
    document.querySelectorAll('.tw-btn').forEach(btn => {
        ...
        if(twList.includes(tw)) btn.disabled = false;
...
document.querySelectorAll('.tw-btn').forEach(btn => {
    btn.addEventListener('click', function(){
        if(this.disabled) return; // ga bisa klik jika disabled
        document.querySelectorAll('.tw-btn').forEach(x => x.classList.remove('bg-[var(--polban-blue)]','text-white','active'));
...
document.querySelector('#loadBtn').addEventListener('click', async function(){
    ...
    json.indikator.forEach(ind => {
        ...
        <tr class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3">
                <input name="realisasi_${ind.id}" class="w-full px-3 py-2 border border-gray-300 rounded-lg"
                    value="${ex ? (ex.realisasi ?? '') : ''}">
            </td>
            <td class="px-4 py-3">
                <input name="kendala_${ind.id}" class="w-full px-3 py-2 border border-gray-300 rounded-lg"
                    value="${ex ? (ex.kendala ?? '') : ''}">
            </td>
            <td class="px-4 py-3">
                <input name="strategi_${ind.id}" class="w-full px-3 py-2 border border-gray-300 rounded-lg"
                    value="${ex ? (ex.strategi ?? '') : ''}">
            </td>
            <td class="px-4 py-3">
                <input name="data_dukung_${ind.id}" class="w-full px-3 py-2 border border-gray-300 rounded-lg"
