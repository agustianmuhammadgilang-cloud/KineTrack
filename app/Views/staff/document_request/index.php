...
<style>
    ...
    .doc-card {
        background: white;
        border-radius: 32px; /* Disamakan dengan form-card sebelumnya */
    ...
    .btn-polban {
        transition: all 0.25s ease-out; /* Durasi lebih cepat & timing function lebih alami */
        background: var(--polban-blue);
        color: white !important;
    }

    .btn-polban:hover {
        /* Hilangkan translateY agar tidak "melompat" */
        background-color: var(--polban-blue-light); /* Hanya transisi warna sedikit terang */
        box-shadow: 0 4px 12px rgba(29, 47, 131, 0.2); /* Shadow yang sangat halus */
        outline: none;
    }

    .btn-polban:active {
        transform: scale(0.98); /* Efek membal (klik) yang sangat tipis */
    }
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-12">
        <div class="flex items-center gap-6">
            ...
            <div>
                ...
                <p class="text-[11px] text-slate-400 font-semibold uppercase tracking-[0.2em] mt-1">Kinetrack — Telusuri dan Ajukan Akses Arsip Digital</p>
            </div>
        </div>
...
<?php elseif ($status === 'pending'): ?>
    ...
    <button disabled
        class="mt-8 w-full py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em]
...
<?php elseif ($status === 'approved'): ?>
    ...
    <div class="mt-8 w-full py-4 text-center text-[10px] font-black uppercase tracking-widest
                bg-green-100 text-green-700 rounded-2xl">
        Sudah Memiliki Akses
    </div>

<?php else: /* belum pernah request ATAU rejected */ ?>
    ...
    <button
        ...
        "
        class="btn-polban mt-8 w-full py-4 rounded-2xl text-[10px]
               font-black uppercase tracking-[0.2em] shadow-lg shadow-blue-900/10">
    ...
    <template x-teleport="body">
        <div x-show="showModal" x-cloak class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            <div x-show="showModal" 
                 ...
                 @click="resetForm()" 
                 class="fixed inset-0 bg-blue-950/60 backdrop-blur-sm"></div>

            <div x-show="showModal" 
                 x-transition:enter="ease-out duration-300" 
                 x-transition:enter-start="opacity-0 scale-95 translate-y-8" 
                 x-transition:enter-end="opacity-100 scale-100 translate-y-0"
                 class="relative bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/20">
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <textarea name="reason" rows="3" required 
                                class="input-premium w-full p-5 rounded-3xl text-sm font-medium text-blue-900 placeholder:text-slate-300" 
                                placeholder="Jelaskan keperluan Anda menggunakan dokumen ini secara profesional..."></textarea>
                        </div>
                        ...
                        <div class="pt-2">
                            <div class="flex items-center justify-between mb-5 px-1">
                                ...
                                <button type="button" 
                                    @click="hasAttachment = !hasAttachment; if(!hasAttachment) fileName = ''"
                                    :class="hasAttachment ? 'bg-blue-900 shadow-blue-200' : 'bg-slate-200'"
                            ...
                            <div x-show="hasAttachment" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
                                <div class="relative group">
                                    <input type="file" name="attachment" @change="handleFile" accept=".pdf,.jpg,.jpeg,.png" 
                                        class="absolute inset-0 opacity-0 cursor-pointer z-10" :required="hasAttachment">
                                    
                                    <div class="input-premium w-full p-8 rounded-[2rem] text-center flex flex-col items-center group-hover:bg-blue-50/30 transition-all border-dashed border-2"
                                        :class="fileName ? 'border-blue-900 bg-blue-50/50' : 'border-slate-200'">
